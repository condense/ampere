...
(defn- make-wrapper
  "Wrapper component that tracks cells and rerender `f` wrappee on their change
   ...
   HACK FIXME
   Things go weird when two wrapped components are mountable in the same place:

       (if condition
         (om/build Component1 {:opts {:cells ...
         (om/build Component2 {:opts {:cells ...

   and init-state/will-mount is not called again on condition flip.
   Thus this hack to produce unique wrapper components."
  []
  (fn Wrapper [[f cursor m] owner]
    (reify
      om/IInitState
      (init-state [_]
...
(defn instrument
  ...
  (if (get-in m [:opts :cells])
    (om/build* (make-wrapper) [f cursor m])
    :om.core/pass))
